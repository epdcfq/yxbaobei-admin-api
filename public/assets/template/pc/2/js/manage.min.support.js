    ...
    if(typeof t == "number" || _moduleId == 351 || _moduleId == 325 || _moduleId == 327 || _moduleId == 423){
        ...
        resizeYS.resizable({
            ...
            resize:function(event,ui){
                ...
                var html ='<div class="module_size" style="position: absolute;top:'+y+'px;left:'+x+'px;"><p>宽：'+moduleWidth+'</p><p>高：'+moduleHeight+'</p></div>';
...
Site.imgShapeShow = function(c,shapetype,imgWidth,imgHeight,radiustext){
    ...
    if(!shapetype ){
        c.find(".pic_shape").css("width","100%");
        return;
    }
        /*c.find("img").css({
        "position":"absolute",
    ...
    if(shapetype==0){
        ...
        c.find(".pic_shape").css({
            "-webkit-mask-box-image": "url(/res/jrfcms/img/module/mobishape"+shapetype+".png)",
            "background-size": "100% 100%"
        });
        //c.find("img").css("height","100%");
        shapetype--;
    ...
    //浮动 锁定 状态下  添加width
    if(c.parents(".box").attr("_float") ==1){
        c.find(".pic_shape").css("width","100%");
    }
    if(c.parents(".box").attr("_lock") ==1){
        c.find(".pic_shape").css("width","100%");
    }
...
Site.floatOut = function(t){
    ...
    var _moduleId = $('#module' + f).parents('.box').find('.configuration').attr('_moduleid');
    //浮动后不需要设置宽度的模块
    var notWidthModuleIdsStr = '325,327,351,605,324,323,604,583,603,563,163,141,122,121,101,15,13,21,16,625,626,';
    if( notWidthModuleIdsStr.indexOf(_moduleId+",") != -1){
        Site.removeModuleStyleCssList(f, [{cls:'', key:'width'}]);
        if(_moduleId == 351){
            var width = $('#module' + f).find("input[name=attr_imageWidth]").val();
            var height = $('#module' + f).find("input[name=attr_imageHeight]").val();
            $('#module' + f).parents(".box").css({
                "width":width,
                "height":height
            });
        }
    ...
    if(_moduleId == 625){
        Site.lineModelResizable(f);
    }
    if( _moduleId == 626){
        Site.iconModelResizable(f);
    }
    if(_moduleId == 351 || _moduleId == 325 || _moduleId == 327 || _moduleId == 423){
    	var resizeYS = b.filter(function(index,obj){
			var moduleId = $(obj).find('.configuration').attr('_moduleid')
			var mcid = $(obj).find('.configuration').attr('_mcid')
			var fun = function(){
				if(moduleId == 325 || moduleId == 327 || moduleId == 351 || moduleId == 423 ){
						...
						if(moduleId == 351){
							// 设置父级宽高
						    var w = $(obj).find('.moduleView').children().find("#jvForm"+f).find("#attrImageWidht").val(),
							h = $(obj).find('.moduleView').children().find("#jvForm"+f).find("#attrImageHeight").val();
							$(obj).width(w).height(h);
							$(obj).find('.moduleView').addClass("threeCss");
							YS.width("100%").height("100%");
                            YS.children().eq(0).width("100%").height("100%");
                            ...
                            var shapetype = $(obj).find('.moduleView').children().find("#jvForm"+f).find("input[name=attr_shapetype]").val();
                            var radiustext = $(obj).find('.moduleView').children().find("#jvForm"+f).find("input[name=attr_radius]").val();
                            Site.imgShapeShow(YS.children().eq(0),shapetype,w,h,radiustext);
		...
		resizeYS.resizable({
			...
			resize:function(event,ui){
                ...
                if(moduleId == 325){
                    var formBox = $(ui.element).find("#jvForm"+mcid);
                    var buttonradiusDef = formBox.find("#buttonradiusDef").val();
                    if(buttonradiusDef != 0 ){//如果为设置过圆角 则修改 圆角  attr_pictureStyle     buttonstyle_03  buttonstyle_06  buttonstyle_07  buttonstyle_11
                        var  buttonstyle = formBox.find("input[name=attr_pictureStyle]").val();
                        if(/buttonstyle_(0[367]|11)/.test(buttonstyle)){
                            var minValue = moduleHeight > moduleWidth?moduleWidth:moduleHeight;
                            var radius = minValue/2;
                            //设置属性值
                            formBox.find("#attr_buttonA1").val(radius);
                            formBox.find("#attr_buttonA2").val(radius);
                            formBox.find("#attr_buttonA3").val(radius);
                            formBox.find("#attr_buttonA4").val(radius);

                            //设置圆角样式
                            formBox.siblings("."+buttonstyle).find("a").css({
                                "border-radius":radius+"px",
                            });

                        }
                        btnStylePadding(formBox,buttonstyle,moduleHeight,moduleWidth);
                    }
                }
			...
			stop:function(event,ui){
					...
					var moduleId = $(ui.element).find('.configuration').attr('_moduleid');
					var mcid = $(ui.element).find('.configuration').attr('_mcid');
                    var elementWidth = $(ui.element).width();
                    var elementHeight = $(ui.element).height();
					// 图片
					if(moduleId == 351){
						var formBox = $(ui.element).find("#jvForm"+f);
						formBox.find("#attrImageHeight").val(ui.size.height);
					...
					if(moduleId == 325){
                        ...
                        if(buttonradiusDef != 0){//如果为设置过圆角 则修改 圆角  attr_pictureStyle     buttonstyle_03  buttonstyle_06  buttonstyle_07  buttonstyle_11
                            var  buttonstyle = formBox.find("input[name=attr_pictureStyle]").val();
                            if(buttonstyle == "buttonstyle_03" || buttonstyle == "buttonstyle_06" || buttonstyle == "buttonstyle_07" || buttonstyle == "buttonstyle_11"){
                                var minValue = elementHeight > elementWidth?elementWidth:elementHeight;
                                var radius = minValue/2;
                            ...
                            btnStylePadding(formBox,buttonstyle,elementHeight,elementWidth);
                        }
						$.ajax({
							url: '/moduleUpdate.jspx',
							type: 'post',
 ...
 * @param mcid
 */
Site.iconModelResizable = function(mcid){
    var b = $('#module'+mcid).parents('.box');
    b.resizable( "destroy" );
    b.resizable({
        handles: "all",
        aspectRatio: 1/ 1,
        autoHide: true,
        start: function (event, ui) {
            var moduleWidth,moduleHeight,iconWidth,iconHeight;
            moduleWidth = $(ui.element).width();
            moduleHeight = $(ui.element).height();
            iconWidth = $(ui.element).find('.icon-model').width();
            iconHeight = $(ui.element).find('.icon-model').height();
            $(ui.element).attr("outWidth",moduleWidth-iconWidth);
            $(ui.element).attr("outHeight",moduleHeight-iconHeight);
        },
        resize: function (event, ui) {
            var mcid = $(ui.element).find('.configuration').attr('_mcid');
            var moduleWidth,moduleHeight,outWidth;
            //添加模块尺寸展示
            moduleWidth = $(ui.element).width();
            outWidth = $(ui.element).attr("outWidth");
            console.log($(ui.element),moduleWidth,outWidth)
            Site.setModuleStyleCss(mcid, '.icon-model', 'font-size', (moduleWidth-outWidth)+"px");
 ...
 * @param mcid
 */
Site.lineModelResizable =function(mcid){
    var b = $('#module'+mcid).parents('.box');
    var lineType = $('#module' + mcid).find(".line_model").attr("lineType");
    var handles = "e,w";
    if(lineType ==2){
        handles = "n,s";
    }
    ...
    b.resizable({
        ...
        autoHide: true,
        classes: {
            "ui-resizable-s": "circle",
            "ui-resizable-e": "circle",
            "ui-resizable-w": "circle",
            "ui-resizable-n": "circle"
        },
        resize: function (event, ui) {
            var mcid = $(ui.element).find('.configuration').attr('_mcid');
            var moduleWidth,moduleHeight;
            ...
            var html ='<div class="module_size" style="position: absolute;top:'+y+'px;left:'+x+'px;">';

            if(lineType ==2) {
                html += '<p>高：'+moduleHeight+'</p>';
                Site.setModuleStyleCss(mcid, '.line_model', 'height', moduleHeight+"px");
            }else{
                html += '<p>宽：'+moduleWidth+'</p>';
                Site.setModuleStyleCss(mcid, '.line_model', 'width', moduleWidth+"px");
            }
            html += '</div>';
            $("#bodyDesign").find(".module_size").remove();
            $("#bodyDesign").append(html);
...
Site.dock = function(t){
	...
	function _dock(f){
        ...
        if(_moduleId == 325){
            ...
            width = $('#module' + f).find("input[name=attr_imageWidth]").val();
            height = $('#module' + f).find("input[name=attr_imageHeight]").val();
            $('#module' + f).find("img").css({
                "width":width,
                "height":height
            });
            $('#module' + f).find(".pic_shape").css("width","");
        }
        //20190121 mxs
        //Site.removeModuleStyleCssList(f, [{cls:'', key:'width'}]);
        Site.removeModuleStyleCssList(f, [{cls:'', key:'width'}]);
        Site.setModuleStyleCssList(f, [{cls:'', key:'width', value: '100%'}]);
	};
    //老的浮动模块使用默认停靠
	if(!hasDock){
		MUTIL.dockDefault(f);
...
Site.lock = function (t) {
    ...
    b.removeAttr('_side');
    b.removeAttr('_float');
    $('#module' + f).removeAttr('_float').removeAttr('_side');
    ...
    //模块宽度为0，则删除模块100%设置 20190813 mxs
    if($("#module" + f).width() ==0){
        Site.removeModuleStyleCssList(f, [{cls:'', key:'width'}]);
        var _moduleId = $('#module' + f).parents('.box').find('.configuration').attr('_moduleid');
        if(_moduleId == 351){
            ...
            $('#module' + f).find(".pic_shape").css("width","100%");
        }
