<?php 
/** 
 * 客户账号管理类
 * 
 */

namespace App\Repositories\UCenter;

use Illuminate\Http\Request;
use App\Models\UCenter\UCenterAccountModel;
use App\Models\UCenter\UCenterCustomerModel;
use App\Repositories\Tools\RuleTool;

use App\Repositories\BaseRepository;

class UCenterAccountRepository extends BaseRepository
{
	protected $account;
	protected $customer;
	protected $accountField;

	public function __construct(UCenterAccountModel $account, UCenterCustomerModel $customer)
	{
		// 创建账号对象
		$this->account = $account;
		// 创建客户对象
		$this->customer = $customer;
		// 创建账号允许操作字段
		$this->accountField = $this->account->filter_field();
	}

	/******************************* [1] 账号 + 客户信息组合方法 *******************************/
	/** 
	 * 1. 创建账号 + 客户信息 (兼容信息存在)
	 * 
	 * @param  		 $paramname description
	 * @param     [type]      $args [description]
	 * @return    [type]            [description]
	 */
	public function createAccountAndCustomer($phone, $args, $fam_account_id=0, $role_id=0)
	{
		$result = ['account'=>[], 'customer'=>[]];
		// 获取创建账号数据
		$args['phone'] 			= $phone;
		$args['fam_account_id'] = $fam_account_id;
		$adata = $this->filterAccountData($args);

		/******** 1.1 创建账号,失败直接返回 ********/
		$this->beginTrans();
		$accountInfo = $this->createAccount($adata['phone'], $adata['fam_account_id'], $adata['openid'], $adata['unionid'], $adata['email'], $adata['username']);
		if ($accountInfo['code'] !== 200) {
			$this->rollbackTrans();
			return $accountInfo;
		}

		/******** 1.2 创建客户信息 ********/
		# 1.2.1 获取账号id和家庭id(未指定家庭账号，默认自己账号id为家庭账号id)
		$account_id = $accountInfo['data']['id'];
		$fam_account_id = $accountInfo['data']['fam_account_id'];
		$fam_account_id = $fam_account_id>0 ? $fam_account_id : $account_id;
		# 1.2.2 创建客户信息
		$customerInfo = $this->createCustomer($account_id, $args, $role_id, $fam_account_id);
		if ($customerInfo['code'] !== 200) {
			$this->rollbackTrans();
			return $customerInfo;
		}

		/****** 1.3 更新主账号客户customer_id字段 ******/
		if (!$accountInfo['data']['customer_id'] && !$role_id) {
			$customer_id = $customerInfo['data']['id'];
			$accountState = $this->updateAccountById($account_id, ['customer_id'=>$customer_id]);
			if ($accountState['code'] !== 200) {
				$this->rollbackTrans();
				return $accountState;
			}
			$accountInfo['data']['customer_id'] = $customer_id;
		}
		# 提交事务
		$this->commitTrans();

		// 1.4 账号转为数组,返回数据
		$accountInfo['customer'] = $customerInfo['data'];

		return $this->codeMsg(200, 'success', $accountInfo);

	}


	/******************************* [2] 账号方法 *******************************/

	/** 
	 * [2.1] 创建账号
	 * 
	 * @param     intger      $fam_account_id   [主账号id]
	 * @param     string      $phone    		[手机号]
	 * @param     string      $openid  			[用户授权openid]
	 * @param     string      $unionid  		[用户授权unionid]
	 * @param     string      $email    		[邮箱]
	 * @param     string      $username 		[登录用户名]
	 * 
	 * @return    [array]                [code=>状态码, msg=>消息内容, data=>返回的数据]
	 */
	public function createAccount($phone='', $fam_account_id, $openid='', $unionid='', $email='', $username='')
	{
		// 获取创建账号数据
		$data = $this->filterAccountData(['fam_account_id'=>$fam_account_id, 'phone'=>$phone, 'unionid'=>$unionid, 'email'=>$email, 'username'=>$username]);

		// 2.1 [校验] 创建账号参数有效性及账号存在性判断
		$verify = $this->accountCreatedRules($data);
		if ($verify['code'] !== 200) {
			return $verify;
		}

		// 3. [校验] 账号是否存在，存在直接返回
		$accountInfo = $this->getAccountByPhone($data['phone']);
		if ($accountInfo['code'] === 200 && $accountInfo['data']) {
			return $accountInfo;
		}

		// 创建账号,失败直接返回
		$accountInfo = $this->account->create($data);
		if (!$accountInfo) {
			return $this->codeMsg(2000, '账号创建失败', $data);
		}

		// 账号转为数组
		$accountInfo = $accountInfo->toArray();
		return $this->codeMsg(200, 'success', $accountInfo);
	}

	/** 
	 * 根据账户id，更新账户信息
	 * 
	 * @param     [int]      $id   [客户表主键id]
	 * @param     [array]    $data [待更新的数据]
	 * 
	 * @return    [type]            [description]
	 */
	public function updateAccountById($account_id, $data)
	{
		if (!$account_id) {
			return $this->codeMsg(1000, '更新账号信息缺少必要参数');
		}

		// 顾虑数据
		$data = $this->filterAccountData($data);
		if (!$data) {
			return $this->codeMsg(1001, '更新账号信息缺少数据');
		}

		// 更新账号信息
		$state = $this->account->where(['id'=>$account_id], $data);
		return $this->codeMsg(200, 'success', ['state'=>$state]);
	}

	/** 
	 * 2.1.1 账号有效性校验
	 * 
	 * @return  [array] [code=200成功,否则失败]
	 */
	public function accountCreatedRules($data)
	{
		// 过滤空数据
		$data = array_filter($data);

		if (!$data || !isset($data['phone'])) {
			return $this->codeMsg(1000, '账号创建失败, 缺少必要参数', $data);
		}

		// 1. 创建账号必须有手机号
		if (!$data['phone'] || !RuleTool::phone($data['phone'])) {
			return $this->codeMsg(1001, '请输入正确手机号', $data);
		}
		// 2. unionid判断(可选)
		if (isset($data['unionid']) && $data['unionid'] && !RuleTool::unionid($data['unionid'])) {
			return $this->codeMsg(1002, '用户授权信息错误', $data);
		}

		return $this->codeMsg(200);
	}

	/** 
	 * 根据手机号获取账号信息
	 * 
	 * @param     [number]      $phone [手机号]
	 * @return    [type]             [description]
	 */
	public function getAccountByPhone($phone, $is_customer=false)
	{
		if (!$phone) {
			return $this->codeMsg(1000, '查询账号缺少手机号');
		}

		// 获取账号信息
		$data = $this->account->where(['phone'=>$phone])->first();

		if ($data) {
			// 增加客户信息返回值
			if ($is_customer && isset($data['id']) && $data['id']) {
				$data['customer'] = [];
				$customerInfo = $this->getAccountMainCustomer($data['id']);
				if ($customerInfo['code'] == 200 && $customerInfo['data']) {
					$data['customer'] = $customerInfo['data'];
				}
			}
		}
		return $this->codeMsg(200, 'success', $data);
	}
	/** 
	 * 2.1.2 账号是否存在，存在返回数据
	 * 
	 * @param     [number]      $phone   [手机号]
	 * @param     [string]      $unionid [用户授权id]
	 * 
	 * @return    [array]               [code=200成功,否则失败]
	 */
	public function existsAccount($phone, $unionid='')
	{
		if (!$phone || !RuleTool::phone($phone)) {
			return $this->codeMsg(2004, '缺少必要正确手机号', ['phone'=>$phone, 'unionid'=>$unionid]);
		}

		$sql = 'select id,phone,unionid from f_ucenter_customer_account where phone=:phone';
		$params = [':phone'=>$phone];
		if ($unionid) {
			$sql .= ' OR unionid=:unionid';
			$params[':unionid'] = $unionid;
		}
		$data = $this->account->queryOne($sql, $params);
		if ($data) {
			return $this->codeMsg(2005, '账号已存在', $data);
		} else {
			return $this->codeMsg(200, '账号不存在', $data);
		}
	}

	/** 
	 * 拼接账号数据
	 * 
	 * @param     [array]      $data [待处理数组数据]
	 * 
	 * @return    [array]
	 */
	protected function filterAccountData($data)
	{
		return [
			'fam_account_id' => $this->getVar('fam_account_id', $data, 0),
			'phone'		=> $this->getVar('phone', $data, ''),
			'openid'	=> $this->getVar('openid', $data, ''),
			'unionid'	=> $this->getVar('unionid', $data, ''),
			'username'	=> $this->getVar('username', $data, ''),
			'email'		=> $this->getVar('email', $data, '')
		];
	}

	/****************************** [3] 客户信息方法 ******************************/
	/** 
	 * 3.1 创建客户信息
	 * 
	 * @param     [int]		$account_id 		[账号id]
	 * @param     [array]   $data              	[客户信息]
	 * @param     [int]     $role_id 			[角色]
	 * @param     [int]     $fam_account_id		[主账号id]
	 * 
	 * @return    [type]                         [description]
	 */
	public function createCustomer($account_id, $data, $role_id=0, $fam_account_id=0)
	{
		// 3.1.1 拼接数据
		$data['account_id']		= intval($account_id);
		$data['fam_account_id'] = $fam_account_id ? intval($fam_account_id) : $account_id;
		$data['role_id'] = intval($role_id);

		// 获取客户表允许字段
		$data = $this->customer->filter_data($data);

		// [规则验证] 客户信息验证
		$verify = $this->ruleCreatedCustomer($data);
		if ($verify['code'] !== 200) {
			return $verify;
		}

		// 根据角色获取客户信息,存在直接返回
		$existsCustomer = $this->getCustomerByRole($data['account_id'], $data['role_id']);
		if ($existsCustomer['code']==200 && $existsCustomer['data']) {
			return $existsCustomer;
		}

		// 创建账号
		$customerInfo = $this->customer->create($data);
		if (!$customerInfo) {
			return $this->codeMsg(2000, '客户信息创建失败', $data);
		}

		// 账号转为数组
		$customerInfo = $customerInfo->toArray();
		return $this->codeMsg(200, 'success', $customerInfo);

	}

	/** 
	 * 根据客户id，更新客户信息
	 * 
	 * @param     [int]      $id   [客户表主键id]
	 * @param     [array]    $data [待更新的数据]
	 * 
	 * @return    [type]            [description]
	 */
	public function updateCustomerById($id, $data)
	{
		if (!$id) {
			return $this->codeMsg(1000, '缺少客户id');
		}

		$data = $this->customer->filter_data($data);
		if (!$data) {
			return $this->codeMsg(1001, '缺少客户更新数据');
		}

		// 更新数据
		$state = $this->customer->where(['id'=>$id])->update($data);

		return $this->codeMsg(200, 'success', ['state'=>$state]);
	}

	/** 
	 * @Author    Hybrid
	 * @DateTime  2020-01-17
	 * @copyright [copyright]
	 * @license   [license]
	 * @version   [version]
	 * @param     [type]      $account_id     [description]
	 * @param     [type]      $fam_account_id [description]
	 * @return    [type]                      [description]
	 */
	public function getAccountMainCustomer($account_id)
	{
		return $this->getCustomerByRole($account_id, 0);
	}


	/** 
	 * [规则] 创建客户信息规则定义
	 * 
	 * @param     [type]      $data [description]
	 * @return    [type]            [description]
	 */
	protected function ruleCreatedCustomer($data)
	{
		if (!$data['account_id']) {
			return $this->codeMsg(1000, '创建客户缺少账号id', $data);
		}
		// 所属家庭必要字段
		$fam_account_id = $this->getVar('fam_account_id', $data, 0);
		if (!$fam_account_id) {
			return $this->codeMsg(1001, '创建客户信息缺少家庭id');
		}

		return $this->codeMsg(200);
	}

	/**
	 * 客户角色是否存在
	 * 
	 * @param     [int]      $role_id        [角色id]
	 * @param     [int]      $fam_account_id [description]
	 * 
	 * @return    [array]
	 */
	public function getCustomerByRole($account_id, $role_id)
	{
		# 家庭账号可重复创建
		if (!$account_id) {
			return $this->codeMsg(1000, '缺少账号id');
		}

		// 查询数据
		$params = ['account_id'=>$account_id, 'role_id'=>$role_id];
		$data = $this->customer->where($params)->first();
		// if ($data) {
		// 	$data = $data->toArray();
		// }

		return $this->codeMsg(200, 'success', $data);;
	}

	/** 
	 * 查询客户列表
	 * @param     [type]      $org_id   [description]
	 * @param     [type]      $args     [description]
	 * @param     [type]      $page     [description]
	 * @param     [type]      $per_page [description]
	 * @return    [type]                [description]
	 */
	public function pageListCustomer($args)
	{
		$where = $this->buildCustomerWhere($args);
		$orderBy = 'cu.id desc';
		$sql = 'SELECT %s FROM f_ucenter_customer cu,f_ucenter_customer_account ca,f_ucenter_customer_trace ct WHERE cu.account_id=ca.id AND ct.customer_id=cu.id AND '.$where;
		// 获取列表数据
		$this->parsePaginate($args);
		$result['list'] = $this->customer->queryAll(sprintf($sql, '*'), [], $orderBy, $this->page, $this->limit);

		// 初始化字段释义
		foreach ($result['list'] as &$info) {
			$this->parseCustomerField($info);
		}

		// 总记录数查询
		$result['total'] = count($result['list']);
		if (!($this->page==1 && $result['total']<$this->limit)) {
			$result['total'] = $this->customer->queryOne(sprintf($sql, 'COUNT(*) as num'))['num'];
		}

		return $this->codeMsg(200, 'success', $result);
	}

	// 组建列表查询条件
	protected function buildCustomerWhere($args)
	{
		$where = [];

		// 门店查询
		if ($org_id = $this->getVar('org_id', $args)) {
			$where[] = ' ct.org_id='.intval($org_id);
		}
		// 姓名查询
		if ($realname = $this->getVar('realname', $args)) {
			$where[] = "(cu.realname LIKE '%".$realname."%' OR cu.nickname LIKE '%".$realname."%')";
		}

		// 手机号查询
		if ($phone = $this->getVar('phone', $args)) {
			$where[] = "ca.phone LIKE '%".intval($phone)."%'";
		}
		// 状态查询
		if (isset($args['status']) && is_numeric($args['status'])) {
			$where[] = 'ct.status='.intval($args['status']);
		} else {
			$where[] = 'ct.status=1';
		}

		return implode(' AND ', $where);
	}

	/** 
	 * 初始化客户字段释义
	 * 
	 * @param     [type]      $data [description]
	 * @return    [type]            [description]
	 */
	public function parseCustomerField(&$data)
	{
		if (!$data) {
			return $data;
		}
		// 性别
		if (isset($data['sex'])) {
			$data['sex_name'] = $this->customer->getEnums('sex', $data['sex']);
		}
		// 状态
		if (isset($data['status'])) {
			$data['status_name'] = $this->customer->getEnums('status', $data['status']);
		}
		if (isset($data['nickname']) && isset($data['realname'])) {
			$data['realname'] = $data['realname'] ? $data['realname'] : $nickname;
		}

		return $data;
	}
}